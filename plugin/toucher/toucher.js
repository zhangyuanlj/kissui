...
(function(doc, exports) {
    "use strict";
    var os = function() {
        var navigator = window.navigator, userAgent = navigator.userAgent, android = userAgent.match(/(Android)[\s\/]+([\d\.]+)/), ios = userAgent.match(/(iPad|iPhone|iPod)\s+OS\s([\d_\.]+)/), wp = userAgent.match(/(Windows\s+Phone)\s([\d\.]+)/), isWebkit = /WebKit\/[\d.]+/i.test(userAgent), isSafari = ios ? navigator.standalone ? isWebkit :/Safari/i.test(userAgent) && !/CriOS/i.test(userAgent) && !/MQQBrowser/i.test(userAgent) :false, os = {};
        if (android) {
            os.android = true;
        ...
        pinch:function(ev) {
            ...
            if (config.pinch) {
                ...
                if (Math.abs(1 - scale) > config.minScaleRate) {
                    ...
                    //手势放大, 触发pinchout事件
                    var scale_diff = 1e-11;
                    //防止touchend的scale与__scale_last_rate相等，不触发事件的情况。
