...
Ks().package("K.Ui", function(Ks){
	this.Paging = function(config){
		var $document = $(document);
		var that = this;
		var public = K.Ui.Paging.prototype;
		var pageHtml = "";                  
		var defaults = {                           
			insertObj : "#page",                 
			pageSize: 10,              
		...
		var registerEvent = function(){
			var prevSelector = defaults.insertObj + " .prev";
			var nextSelector = defaults.insertObj + " .next";
			var numSelector = defaults.insertObj + " .num";
			$document.off(prevSelector).on("click", prevSelector, function(e){
				that.goPrev();
				e.stopPropagation();
			});
			$document.off(nextSelector).on("click", nextSelector, function(e){
				that.goNext();
				e.stopPropagation();
			});
			$document.off(numSelector).on("click", numSelector, function(e){
